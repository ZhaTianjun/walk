...
#include <chrono>
using namespace std;
AstarPlanner::AstarPlanner( ):GLX_SIZE(6),GLY_SIZE(7){
    data.clear();
...
AstarPlanner::AstarPlanner(vector<vector<int>>  data_ ){
    data = data_;
    GLY_SIZE=data_.size();
...
bool AstarPlanner::isOccupied(int x,int y){
...
double AstarPlanner::getHeu(GridNodePtr node1, GridNodePtr node2){
    double dx=fabs(node1->x - node2->x);
    double dy=fabs(node1->y - node2->y);
    return dx+dy+(sqrt(2)-2)*min(dx,dy);
}
void AstarPlanner::GetNeigborsets(GridNodePtr currentPtr,vector<GridNodePtr>& neighborPtrSets,
...
void AstarPlanner::AstarGraphSearch(pair<int,int> start_,pair<int,int> goal_){
